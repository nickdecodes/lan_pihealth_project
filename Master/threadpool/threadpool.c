...
void threadpool_init(threadpool_t *pool, int idle_threads, int max_threads) {
    ...
    pool->quit =0;
    pool->tid = (pthread_t *)malloc(sizeof(pthread_t) * max_threads);

    //创建空闲线程
   int i = 0;
   for(; i < max_threads; i++) {
        pthread_create(&(pool->tid[i]), NULL, thread_routine, pool);
    ...
    if(pool->counter > 0) {
        ...
        if(pool->idle > 0) {
            ...
            for (int i = 0; i < pool->max_threads; i++) {
                pthread_join(pool->tid[i], NULL);//以参与的方式回收线程资源
            }
        }
        //正在执行任务的线程，等待他们结束任务
        while(pool->counter) {
